...
def run():
    ...
    while running:
        ...
        if current_player._is_human:
            judge.validate_move(current_player)  # Validate the move using Judge
        else:
            # CPU player makes a move
            current_player.make_move(judge._board)

        # Check for a winner or draw
        if judge.check_winner(current_player._symbol):
            judge.create_board()
            print(f'{current_player._name} wins! Congratulations!')
            running = False
